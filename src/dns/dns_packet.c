...
typedef struct header {
  ...
  */
    ...
    uint16_t
        NSCOUNT; // 16 bit int Number of name server RR in the authority section
...
Header* __init__header(uint16_t ID,
    uint8_t QR,
    uint8_t opcode,
    uint8_t AA,
    uint8_t TC,
    ...
    uint8_t* RDATA)
{
    ...
    self->__len__ = (strlen(self->NAME) + 1) + self->RDLENGTH + sizeof(self->RRTYPE) + sizeof(self->CLASS) + sizeof(self->TTL) + sizeof(self->RDLENGTH);
    self->__del__ = *__del__rrformat;
    last_result = NOERROR;
    return self;
}

Answer* __init__answer(char* NAME,
    uint16_t RRTYPE,
    uint16_t CLASS,
    int32_t TTL,
    uint16_t RDLENGTH,
    uint8_t* RDATA)
{
    return (Answer*)__init__rr(NAME, RRTYPE, CLASS, TTL, RDLENGTH, RDATA);
}

Authority* __init__authority(char* NAME,
    uint16_t RRTYPE,
    uint16_t CLASS,
    int32_t TTL,
    uint16_t RDLENGTH,
    uint8_t* RDATA)
{
    return (Authority*)__init__rr(NAME, RRTYPE, CLASS, TTL, RDLENGTH, RDATA);
}
Additional* __init__additional(char* NAME,
    uint16_t RRTYPE,
    uint16_t CLASS,
    int32_t TTL,
    uint16_t RDLENGTH,
    uint8_t* RDATA)
{
    return (Additional*)__init__rr(NAME, RRTYPE, CLASS, TTL, RDLENGTH, RDATA);
}

DNSMessage* __init__message(Header* head,
    Question** quest,
    Answer** ans,
    Authority** auth,
    Additional** addl)
...
Header* bytes_to_header(uint8_t* packet)
{
  ...
  */
    ...
    Header* message_header = __init__header(ID, QR, opcode, AA, TC, RD, RA, RES, AD, CD, RCODE,
...
RRFORMAT* bytes_to_resource_record(uint8_t* packet, uint32_t* cur_loc)
{
  ...
  */
    ...
    *cur_loc += 2;
    ...
    TTL = packet[*cur_loc];
    TTL = (TTL << 24) | ((int32_t)packet[*cur_loc + 1] << 16) | ((int32_t)packet[*cur_loc + 2] << 8) | packet[*cur_loc + 3];
    *cur_loc += 4;
